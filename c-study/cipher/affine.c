/**
 * @file
 * @brief An [affine cipher] is a letter substitution cipher that uses a linear transformation to substitute
...
void test_string(const char *s, const char *ciphertext, int a, int b)
{
    ...
    assert(strcmp(copy, s) == 0);  // assert that we got the same string we started with
...
static void tests()
{
    ...
    affine_encrypt(msg, key);
    printf("encrypt:%s\n", msg);

    affine_decrypt(msg, key);
    printf("decrypt:%s\n", msg);
